...
local Tabs = {}
local TabButtons = {}
local GameTabButtons = {}
local GameList = {"Arsenal", "MM2", "Rivals", "Phantom Forces", "Counter Blox", "Bad Business", "Tower Defense", "BedWars"}
...
local function CreateMenu()
    ...
    local function ShowTab(tabName)
        ...
        for name, b in pairs(TabButtons) do
            if name == tabName then
                ...
                b.BackgroundColor3 = Color3.fromRGB(17, 19, 23)
            else
                b.TextColor3 = Color3.fromRGB(110, 115, 125)
                b.BackgroundColor3 = Color3.fromRGB(8, 9, 11)
            end
        end
    end
    ...
    local function CreateSidebarTab(name)
        CreateTab(name)
        local b = Instance.new("TextButton", SidebarScroll)
    ...
    local function CreateGameButton(name)
        local b = Instance.new("TextButton", SidebarScroll)
        b.Size = UDim2.new(1, -4, 0, 24)
        b.BorderSizePixel = 0
        b.BackgroundColor3 = Color3.fromRGB(8, 9, 11)
        ...
        b.TextXAlignment = Enum.TextXAlignment.Left
        Instance.new("UICorner", b).CornerRadius = UDim.new(0, 4)
        GameTabButtons[name] = b
        b.MouseButton1Click:Connect(function()
            if KeyVerified then
                CurrentGame = name
                for _, btn in pairs(GameTabButtons) do
                    btn.BackgroundColor3 = Color3.fromRGB(8, 9, 11)
                end
                b.BackgroundColor3 = Color3.fromRGB(0, 110, 220)
                
                -- Обновляем заголовок
                local header = MenuFrame:FindFirstChild("Frame")
                if header then
                    local title = header:FindFirstChild("TextLabel")
                    if title then
                        title.Text = "    NEVERLOSE | " .. name
                    end
                end
                print("✅ Выбрана игра: " .. name)
            else
                -- Если ключ не введен, показываем окошко с ключом
                ...
                kSub.Size = UDim2.new(1, 0, 0, 25)
                kSub.Position = UDim2.new(0, 0, 0, 40)
                kSub.BackgroundTransparency = 1
                kSub.Text = T("subtitle")
                kSub.TextColor3 = Color3.fromRGB(150, 155, 165)
                kSub.Font = Enum.Font.GothamSemibold
                kSub.TextSize = 11
                local tBg = Instance.new("Frame", keyFrame)
                tBg.Size = UDim2.new(0.9, 0, 0, 35)
                tBg.Position = UDim2.new(0.05, 0, 0, 75)
                tBg.BackgroundColor3 = Color3.fromRGB(17, 19, 23)
                Instance.new("UICorner", tBg).CornerRadius = UDim.new(0, 4)
                local kInput = Instance.new("TextBox", tBg)
                kInput.Size = UDim2.new(1, -10, 1, 0)
                kInput.Position = UDim2.new(0, 5, 0, 0)
                kInput.BackgroundTransparency = 1
                kInput.PlaceholderText = T("placeholder")
                kInput.Text = ""
                kInput.TextColor3 = Color3.fromRGB(255, 255, 255)
                kInput.Font = Enum.Font.GothamSemibold
                kInput.TextSize = 12
                local sBtn = Instance.new("TextButton", keyFrame)
                sBtn.Size = UDim2.new(0.42, 0, 0, 32)
                sBtn.Position = UDim2.new(0.05, 0, 0, 125)
                sBtn.BackgroundColor3 = Color3.fromRGB(0, 110, 220)
                sBtn.Text = T("activate")
                sBtn.Font = Enum.Font.GothamBold
                sBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
                sBtn.TextSize = 11
                Instance.new("UICorner", sBtn).CornerRadius = UDim.new(0, 4)
                local gBtn = Instance.new("TextButton", keyFrame)
                gBtn.Size = UDim2.new(0.42, 0, 0, 32)
                gBtn.Position = UDim2.new(0.53, 0, 0, 125)
                gBtn.BackgroundColor3 = Color3.fromRGB(17, 19, 23)
                gBtn.Text = T("buy")
                gBtn.Font = Enum.Font.GothamBold
                gBtn.TextColor3 = Color3.fromRGB(0, 110, 220)
                gBtn.TextSize = 11
                ...
                statusLbl.Text = T("waiting")
                statusLbl.TextColor3 = Color3.fromRGB(110, 115, 125)
                ...
                sBtn.MouseButton1Click:Connect(function()
                    ...
                    task.spawn(function()
                        ...
                        if keyValid then
                            KeyVerified = true
                            statusLbl.Text = "✅ Ключ подтвержден! Till: " .. expiryInfo
                            statusLbl.TextColor3 = Color3.fromRGB(0, 200, 100)
                            task.wait(0.5)
                            keyGui:Destroy()
                            -- Обновляем SubLbl
    ...
    -- ========== СОЗДАНИЕ САЙДБАРА ==========
    local gameHeader = Instance.new("TextLabel", SidebarScroll)
    gameHeader.Size = UDim2.new(1, 0, 0, 20)
    gameHeader.BackgroundTransparency = 1
    gameHeader.Text = "  " .. T("select_game")
    gameHeader.TextColor3 = Color3.fromRGB(75, 80, 90)
    gameHeader.Font = Enum.Font.GothamBold
    gameHeader.TextSize = 8.5
    gameHeader.TextXAlignment = Enum.TextXAlignment.Left

    for _, gameName in ipairs(GameList) do
        CreateGameButton(gameName)
    end
    ...
    CreateToggle(gAA, "Рысканье (Yaw)", "YawJitter")
    ...
    CreateToggle(gPl, "Цвета ролей (MM2)", "ESP_TeamColors")
    ...
    CreateToggle(gMain, "Автофарм", "AutoFarm")
    ...
    infoLabel.Text = "NEVERLOSE LOADER\n" .. T("version") .. "\nTill: " .. TillText
...
print("✅ NEVERLOSE LOADER ЗАГРУЖЕН!")
print("📌 Выберите игру в сайдбаре и введите ключ")
